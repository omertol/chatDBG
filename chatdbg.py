...
def load_data(file_path):
    try:
        ...
        new_column_names = ['book_id', 'type', 'unit', 'headline', 'from_date', 'to_date', 'additional_info', 'additional_info_2']
...
def preprocess_data(original_df):
    ...
    df['combined'] = df.apply(lambda row: f"Headline: {row['headline']}. Text: {row['combined_info']}" if row['combined_info'] else f"Headline: {row['headline']}", axis=1)
